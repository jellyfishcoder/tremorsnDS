...
Button::Button(int _oamId, int _affId, MathVector2D<int> _position, MathVector2D<int> _scale) {
	...
	oamSet(&oamMain,					// Only on touch screen
		this->oamId,					// oamId
		this->position.x - 32*fixedToFloat(this->scale.x, 8),	// Left corner X position
		this->position.y - 16*fixedToFloat(this->scale.y, 8),	// Left corner Y position
		0,						// Priority 
		this->oamId,					// Alpha
...
bool Button::CheckTouch(touchPosition _touch) {
	...
	int left = this->position.x - 32*fixedToFloat(this->scale.x, 8);
	int right = this->position.x + 32*fixedToFloat(this->scale.x, 8);
	int up = this->position.y - 16*fixedToFloat(this->scale.y, 8);
	int down = this->position.y + 16*fixedToFloat(this->scale.y, 8);
